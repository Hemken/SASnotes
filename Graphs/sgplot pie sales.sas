...
proc sgplot data=piesales;
	vbar flavor / freq=pies_sold;
run;

proc sgplot data=piesales;
	vbar flavor / response=pies_sold;
run;

ods listing style=journal;

proc sgplot data=piesales;
	vbar flavor / response=pies_sold stat=mean;
run;

proc sgplot data=piesales;
	vbar flavor / group=year response=pies_sold;
run;

proc sgpanel data=piesales;
	panelby flavor /layout=columnlattice columns=4;
	vbar year / response=pies_sold;
run;

proc sgpanel data=piesales;
	panelby flavor /layout=columnlattice columns=4;
	vbar year / group=bakery response=pies_sold;
run;

proc sgplot data=piesales;
	hbox pies_sold / group=bakery;
run;

proc sgplot data=piesales;
	dot bakery / response=pies_sold stat=mean 
		limitstat=stderr numstd=1.96;
run;
